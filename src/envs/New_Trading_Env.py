...
from src.rewards.reward_functions import simple_portfolio_value_change

class MultiStockTradingEnv(gym.Env):
    """
    A Gymnasium environment for multi-stock trading.

    This environment simulates trading multiple stocks based on historical price data,
    technical indicators, and macroeconomic features. It is designed to be used
    with reinforcement learning agents from libraries like Stable-Baselines3.

    Attributes:
        df (pd.DataFrame): Preprocessed DataFrame containing stock prices, indicators, and features.
        num_stocks (int): The number of different stocks to trade.
        initial_amount (float): The initial cash balance for trading.
        buy_cost_pct (list[float]): Percentage cost for buying each stock.
        sell_cost_pct (list[float]): Percentage cost for selling each stock.
        hmax_per_stock (list[int]): Maximum number of shares that can be held or traded per stock.
        reward_scaling (float): A factor to scale the raw reward.
        tech_indicator_list (list[str]): List of technical indicator names (base names, e.g., 'macd', 'rsi').
                                         The environment expects columns like 'macd_0', 'rsi_0', etc.
        risk_penalty (float): A penalty factor for risk (currently not used with simple reward).
        lookback_window (int): Number of past steps to consider for some calculations (e.g., VWAP, not currently used for reward).
        make_plots (bool): If True, enables plotting (currently not implemented).
        print_verbosity (int): Controls frequency of print statements during rendering.
        stop_loss_threshold (float): Percentage drop from VWAP to trigger a stop-loss sale.
        max_steps_per_episode (int): The maximum number of steps in an episode before truncation.
        training (bool): If True, sets the environment in training mode (e.g., affects max_steps).
        seed (int, optional): Random seed for reproducibility.
        # ... (other attributes like cash, shares, portfolio_value, etc.)
    """
    ...
    def __init__(
        ...
        """
        Initializes the MultiStockTradingEnv.

        Args:
            df: DataFrame with stock data.
            num_stocks: Number of stocks.
            initial_amount: Starting cash.
            buy_cost_pct: List of buy costs per stock.
            sell_cost_pct: List of sell costs per stock.
            hmax_per_stock: List of max shares per stock.
            reward_scaling: Factor to scale rewards.
            tech_indicator_list: List of technical indicator names.
            risk_penalty: Penalty for risk (currently not used).
            lookback_window: Lookback window for features.
            make_plots: Enable plotting (not implemented).
            print_verbosity: Verbosity for rendering.
            stop_loss_threshold: Stop-loss percentage.
            max_steps_per_episode: Max steps per episode.
            training: Training mode flag.
            # reward_params: Parameters for reward calculation (currently commented out).
            seed: Random seed.
        """
        super().__init__()
        self.seed = seed
    ...
    def _initialize_scalers(self):
        ...
        # Fit scalers
        # TODO: Data Leakage Warning!
        # The scalers are currently fit on the entire DataFrame `self.df` passed during initialization.
        # If `self.df` includes data intended for testing or validation, this constitutes data leakage.
        # For proper evaluation, scalers should be fit ONLY on the training portion of the data.
        # This might require adjusting the data pipeline or how the environment receives data splits.
        price_cols = []
        tech_cols = []
        for stock in range(self.num_stocks):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        Args:
            ...
            options (dict, optional): Additional options (not currently used).
        ...
        # self.weights = self.initialize_weights([1.0, 1.0, 1.0, 1.0]) # Old reward system
    ...
    def _get_obs(self):
        ...
        - Normalized cash available.
        - Normalized shares held for each stock.
        - Scaled price features (OHLCV) for each stock.
        - Scaled technical indicator features for each stock.
        - Scaled macroeconomic features.

        Returns:
            np.ndarray: The current observation array.
        """
    ...
    def step(self, action: np.ndarray):
        ...
        Args:
            action (np.ndarray): The action to take for each stock (e.g., buy/sell percentage).
        ...
        Returns:
            tuple: A tuple containing (observation, reward, terminated, truncated, info).
                   ...
                   - reward (float): The amount of reward returned after taking the action.
                   - terminated (bool): Whether the episode has ended (e.g., due to a terminal state).
                   - truncated (bool): Whether the episode has been cut short (e.g., due to time limit).
        ...
        self.current_step += 1
        terminated = False  # Placeholder: Set to True if other conditions cause episode end
        ...
        else: # Episode ended (terminated or truncated)
        ...
        # Calculate reward using new simple function
        reward = simple_portfolio_value_change(self.portfolio_value, prev_value)

        # Update weights dynamically - Commented out as part of old reward system
        # volatility = np.std(self.daily_returns[-self.reward_params['lookback_window']:]) # Uses old reward_params
...
#        return reward
    ...
    def render(self, mode="human"):
        ...
        Currently, this prints information to the console based on `print_verbosity`.

        Args:
            mode (str): The mode to render with (e.g., "human").
        """
...
#
#        penalty_drawdown = w_drawdown * (k_d * (drawdown ** delta) / softplus__drawdown)
#
